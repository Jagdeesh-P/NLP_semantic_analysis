...
def home():
    ...
    return render_template('home1.html', videos=videos)
...
def search():
    ...
    if semantic_result and 'videos' in semantic_result:
        ...
        try:
            ...
            if video_list:
                ...
                return render_template('search_results1.html', keyword=keyword, videos=video_list)
    ...
    if not video_list:
        print('Re Search')
        return render_template('search_results1.html', keyword=keyword, videos=None)
...
def like_video(video_id):
    video = mongo.db.video_postives.find_one({"_id": ObjectId(video_id)})

    # Check if 'likes' is a string, and convert to integer if needed
    if isinstance(video['likes'], str):
        try:
            video['likes'] = int(video['likes'])
            mongo.db.video_postives.update_one({"_id": ObjectId(video_id)}, {"$set": {"likes": video['likes']}})
        except ValueError:
            return "Error: 'likes' field is not a valid number."

    # Now safely apply the $inc operator
